			...
			if ( isset( $args[ $key ] ) ) {
				...
				if ( isset( $array['min_chars'] ) && $array['min_chars'] > 0 ) {
					if ( $args[ $key ] && strlen( utf8_decode( $args[ $key ] ) ) < $array['min_chars'] ) {
						if ( ! empty( $array['label'] ) ) {
							UM()->form()->add_error( $key, sprintf( __( 'Your %s must contain at least %s characters', 'ultimate-member' ), strtolower($array['label']), $array['min_chars'] ) );
						}
						elseif ( ! empty( $array['placeholder'] ) ) {
							UM()->form()->add_error( $key, sprintf( __( 'Your %s must contain at least %s characters', 'ultimate-member' ), strtolower($array['placeholder']), $array['min_chars'] ) );	
						} 
						else {
				...
				if ( isset( $array['max_chars'] ) && $array['max_chars'] > 0 ) {
					if ( $args[ $key ] && strlen( utf8_decode( $args[ $key ] ) ) > $array['max_chars'] ) {
						if ( ! empty( $array['label'] ) ) {
							UM()->form()->add_error( $key, sprintf( __( 'Your %s must contain less than %s characters', 'ultimate-member' ), strtolower($array['label']), $array['max_chars'] ) );
						}
						elseif ( ! empty( $array['placeholder'] ) ) {
							UM()->form()->add_error( $key, sprintf( __( 'Your %s must contain less than %s characters', 'ultimate-member' ), strtolower($array['placeholder']), $array['max_chars'] ) );	
						} 
						else {
